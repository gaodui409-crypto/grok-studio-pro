...
阅读器读取本机上传的漫画页和生成结果。“漫画下载”是独立的 PicaComic 入口，下载后的图片可再导入漫画工具。

同一标签页内的文生图、图生图、同人图和漫画图片任务，按渠道、接口地址和凭据共用设置中的并发上限；等待中的任务可取消。提高并发不会提高仍在运行批次的旧上限。其他标签页、视频任务和外部软件需要上游统一限流。

### 8. PicaComic 漫画下载

1. 打开“漫画下载”，登录本人 PicaComic 账号。密码用于本次登录，会话 Token 保存在当前浏览器中。
2. 先在“下载队列”选择保存目录，再从搜索、收藏或排行榜打开漫画，下载单章或全部章节。
3. 选择目录后，图片写入带稳定 ID 的漫画和章节子目录，按页码命名，并生成 `comic.json`；未选择目录时逐张交给浏览器下载。
4. 队列支持暂停、恢复、取消、重试和清理。“已下载”页选择保存目录后会扫描 `comic.json`。

目录写入需要支持 File System Access API 的浏览器及目录授权，例如本机 Chrome/Edge。每个任务保存自己的目录句柄，刷新后未完成任务会恢复为暂停状态，点击恢复时可能需要重新授权原目录；新任务需重新选择保存目录。更换目录只影响之后新建的任务。浏览器下载兜底无法确认文件是否实际落盘，长章节建议先选择目录，避免被浏览器拦截多文件下载。

Pica 请求经应用服务端中转。中转限制请求路径、图片域名、重定向及响应大小，但尚未配备应用级访问认证和部署级限流，不建议直接开放到公网。只下载自己有权获取的内容，勿分享账号 Token。

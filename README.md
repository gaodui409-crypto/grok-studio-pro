...
- 漫画工具：批量上色，或先识别翻译文字，再将译文嵌回图片，并可作为漫画连续翻页阅读。
...
- 双击 `启动.bat`：启动开发服务。首次运行且缺少依赖时，脚本会先执行 `npm install`。
- 双击 `测试.bat`：运行项目的完整单元测试，窗口会显示通过数或失败信息。

两个脚本都会自动切换到项目目录，因此从快捷方式或其他目录启动也可以正常工作。在命令行中运行 `启动.bat --host 0.0.0.0`，可将参数继续传给 Vite。
...
阅读器：点“阅读”，或点某一行的“查看大图”从那一页开始读。

- 单页模式一页一页翻，方向键、Home 和 End 都可用，到首尾就停，不会绕回去。
- 连页模式竖向连续滚动，页码跟着当前页走。
- 随时切换“结果 / 原图”对比，展开该页译文，或下载当前页。
- 还没生成结果的页不会被跳过，会显示原图并说明原因，页码始终保持整本的顺序。

阅读器只读本机上传的漫画页和它们的生成结果。项目不内置漫画站的搜索、登录或抓取：那需要伪装成对方的客户端并重放它的签名密钥去批量拉取版权作品，也需要把第三方账号密码存进浏览器，两者都不做。要读别处的漫画，请先自行下载到本地再上传。

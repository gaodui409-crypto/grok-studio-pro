          ...
          {pages.map((p, i) => (
            ...
            >
              ...
              <div className="flex items-center justify-between p-2 text-[11px]">
                <span className="truncate text-muted-foreground">#{i + 1} · {p.name}</span>
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => move(i, -1)} className="opacity-70 hover:opacity-100"><ArrowUp className="h-3.5 w-3.5" /></button>
                  <button onClick={() => move(i, 1)} className="opacity-70 hover:opacity-100"><ArrowDown className="h-3.5 w-3.5" /></button>
                  <button onClick={() => remove(i)} className="opacity-70 hover:text-destructive"><X className="h-3.5 w-3.5" /></button>
                </div>
              </div>
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
      <div className="space-y-5 rounded-2xl border border-border/60 bg-card p-5 shadow-card">
        ...
        <div className="flex flex-wrap justify-end gap-2">
          <Button variant="secondary" disabled={!pages.some((p) => p.resultUrl)} onClick={downloadAll}>
            <Download className="mr-2 h-4 w-4" /> 打包下载
          </Button>
          <Button onClick={handleRun} disabled={running} className="bg-gradient-primary text-primary-foreground shadow-glow">
          ...
          <Select value={preset} onValueChange={(v) => set({ preset: v })}>
            ...
            <SelectContent>
              {LANG_PRESETS.map((l, i) => (
                <SelectItem key={i} value={String(i)}>{l.from} → {l.to}</SelectItem>
              ))}
